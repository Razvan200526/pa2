...
inline string appendZero(string str, int n){
	while(n>0){
		str = "0" + str;
		n--;
	}
...
inline string multiplyXY(string x, string y){
	int product;
	product = ((int)x[0] - '0') * ((int)y[0] - '0');
	return to_string(product);
}

inline string addXY(string x, string y){
	if(x.length() > y.length()){
		y = appendZero(y, x.length() - y.length());
	}else{
		x = appendZero(x, y.length() - x.length());
...
inline string subtractXY(string x, string y){
	if(x.length() > y.length()){
		y = appendZero(y, x.length() - y.length());
	}else{
		x = appendZero(x, y.length() - x.length());
...
inline string removeZero(string str){
...
inline string multiply10(string str, int n){
	while(n>0){
		str = str + "0";
		n--;
	}
